...
int main()
{
  ...
  while (tests--)
  {
    ...
    if (best >= 2)
      maxLength++;
    std::cout << best << std::endl; //<< " " << maxLength << std::endl;
  }
...
// int vectorRangeSum(int lower, int upper, vector<int> v){
//     int sum;
...
//     int length = 0;
//     int largest = 0;
//     int lastj = prime_num.size();
//     int n = prime_num.size();

//     for(int i = 0; i<n; i++){
//         for(int j = i+length; j<lastj; j++){
//             int sol = vectorRangeSum(i, j, prime_num);
//             if(sol < 1000000){
//                 length = j-i;
//                 largest = sol;
//             }
